...
class Feed {
    ...
    function getAllFeedsForOneUser ($id_user) {
        ...
        $request = $connexion->prepare("
            SELECT post.title, post.id AS post_id, post.content, post.picture, user.firstname, user.lastname, user.picture AS picture_user 
            FROM post
            JOIN user
    ...
    function getOneFeed($post_id){
        ...
        ");
        // j'exécute la requête
        $request->execute([':post_id' => $post_id]);
        // je récupère tous les résultats dans users
        $feed = $request->fetch(PDO::FETCH_ASSOC);
        // je ferme la connection
        $connexion = null;
