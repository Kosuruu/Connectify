...
class Publication {
    ...
    function addPublicationInGroup() {
        ...
        if($member['status'] == 1){
            ...
            if(move_uploaded_file($picture['tmp_name'], $targetPath)) {
                ...
                // je prépare la requête
                $sql = "INSERT INTO publication (publication.title, publication.content, publication.picture,  publication.group_id, publication.author_id) 
                        VALUES (:title, :content, :picture, :group_id,  :author_id)
                        ";
    
                $request = $connection->prepare($sql);

                $picturePath = 'http://localhost:4000/images/publications/' . $fileName;
                ...
                header('Location: http://localhost:3000/Page/group.php?id=' . $group_id . '?message=' . urlencode($message));
                exit;
            }
