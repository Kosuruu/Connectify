...
if (isset($_GET['id'])) {
    ...
    $feedsInfo = "http://localhost:4000/profile/feed/get/" . $id;
    $jsonfeeds = file_get_contents($feedsInfo);
    $feeds = json_decode($jsonfeeds, true);
...
<main class="main" overflow="hidden">
    ...
    <div class="Publication" id="contentPublication">
        ...
        <?php foreach($feeds as $feed): ?>
            <a href="../Page/onepublication.php?id=<?=$feed['post_id'] ?>">
                <div>
                    <img class="img1" src="<?=$feed['picture']?>"></img>
                    <p><?=$feed['title']?></p>
                </div>
            </a>
        <?php endforeach; ?> 
            ...
            <button class=" friendList">
                    <div class="profileInvitation sliderFriendsContent">
                        <?php foreach($searchRelations as $searchR): ?>
                            <a href="../Page/profile.php?id=<?=$searchR['id']?>">
