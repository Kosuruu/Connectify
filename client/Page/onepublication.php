...
if (isset($_GET['id'])) {
	...
	$post_id = $_GET['id'];
	$getPost = "http://localhost:4000/profile/oneFeed/get/" . $post_id;
	$jsonPost = file_get_contents($getPost);
	...
	<div class="onePublication">
		<div class="containerPublication">
			<div class="containerProfile">
				<img class="imageProfile" src="<?=$post['picture_user']?>">
				<div class="profile">
						<h3 class="textWhite"><?=$post['firstname']?> <?=$post['lastname']?></h3>
				</div>
			</div>
			<h3 class="titrePublication textWhite"><?=$post['title']?></h3>
			<img class="imagePublication" src="<?=$post['picture']?>">
			<p><?=$post['content']?></p>
		</div>
	</div>
